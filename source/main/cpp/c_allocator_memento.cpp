...
namespace ncore
{
    namespace memento
    {
        ...
        class NullPrint : public memento_reporter_t
        {
        public:
        ...
        struct Instance
        {
            ...
            memento_t*          m_memento;
            memento_reporter_t* m_report;
            alloc_t*            m_allocator;
            memento_handler_t*  m_event_handler;
        ...
        static void show_block(BlockHeader* b, int space, memento_reporter_t* printer)
        {
        ...
        static void blockDisplay(BlockHeader* b, int n, memento_reporter_t* printer)
        {
            n++;
            while (n > 40)
            {
        ...
        static int s_list_block(BlockHeader* b, void* arg, memento_reporter_t* printer)
        {
            int* counts = (int*)arg;
        ...
        static int s_list_newblock(BlockHeader* b, void* arg, memento_reporter_t* printer)
        {
        ...
        {
            memento_reporter_t* m_printer;

        public:
            inline listnewblock_functor(memento_reporter_t* printer) : m_printer(printer) {}

            virtual int dofunc(BlockHeader* blkh, void* arg) { return s_list_newblock(blkh, arg, m_printer); }
        };
        ...
        static void doNestedDisplay(NestedBlock* b, int depth, memento_reporter_t* printer)
        {
        ...
        public:
            ...
            virtual int dofunc(BlockHeader* blkh, void* arg) { return m_instance->internal_check_allocedblock(blkh, arg); }
        };
        ...
        public:
            ...
            virtual int dofunc(BlockHeader* blkh, void* arg) { return m_instance->internal_check_allalloced(blkh, arg); }
        };
        ...
        public:
            ...
            virtual int dofunc(BlockHeader* blkh, void* arg) { return m_instance->internal_check_allfreed(blkh, arg); }
        };
        ...
        public:
            ...
            virtual int dofunc(BlockHeader* blkh, void* arg) { return Memento_containsAddr(blkh, arg); }
        };
    ...
    class memento_allocator : public memento_t
    {
    public:
        virtual const char* name() const { return TARGET_FULL_DESCR_STR " memento debug allocator"; }

        virtual void* allocate(u32 size, u32 alignment, const char* label, int var)
        {
        ...
        virtual void set_reporter(memento_reporter_t* reporter) { m_memento.m_report = reporter; }

        virtual void set_handler(memento_handler_t* handler) { m_memento.m_event_handler = handler; }

        virtual void paranoia(int p) { m_memento.set_paranoia(p); }

        virtual void paranoidAt(int sequence) { m_memento.paranoid_at(sequence); }

        virtual void breakAt(int sequence) { m_memento.break_at(sequence); }
        ...
        virtual void list_blocks() { m_memento.list_blocks(); }

        virtual void check() { m_memento.check(); }

        virtual bool check_block(void* p) { return m_memento.check_block(p) == 0; }

        virtual void check_all_memory() { m_memento.check_all_memory(); }

        virtual void break_on_free(void* p) { m_memento.break_on_free(p); }

        ///
        /// alloc_t interface
        ///
        virtual void* allocate(u32 size, u32 alignment) { return m_memento.mmalloc((u32)size, alignment); }

        virtual void* reallocate(void* ptr, u32 size, u32 alignment) { return m_memento.mrealloc(ptr, (u32)size); }

        virtual void deallocate(void* ptr) { return m_memento.mfree(ptr); }
        ...
        void* operator new(u32 num_bytes) { return NULL; }
    ...
    private:
        virtual ~memento_allocator() {}
    };

    memento_t* gCreateMementoAllocator(alloc_t* allocator)
    {
        void*              mem                       = allocator->allocate(sizeof(memento_allocator), sizeof(void*));
        memento_allocator* memento_allocator         = new (mem) memento_allocator();
